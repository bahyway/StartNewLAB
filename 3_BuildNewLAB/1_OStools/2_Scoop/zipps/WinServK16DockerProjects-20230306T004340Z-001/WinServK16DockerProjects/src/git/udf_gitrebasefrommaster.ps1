...
.DESCRIPTION
  This script makes sure master is up to date, then rebases the current branch.
...
$sLogName = "gitrebasefrommaster.log"
...
if($currentBranch -ne "master") {
    ...
    Log-Write -LogPath $sLogFile -LineValue "Rebasing from master branch"
    git rebase master
    ...
    if(($allbranches | where{$_ -eq $env:COMPUTERNAME})) {
		Log-Write -LogPath $sLogFile -LineValue "Checkout $env:COMPUTERNAME branch"
		...
		Log-Write -LogPath $sLogFile -LineValue "Creating new branch from master branch"
